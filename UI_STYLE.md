...
- Default text must always use the normal foreground color (`--fg`, `text-fg`, or equivalent).
- `text-fg` is the baseline for all primary content, including:
  - headers and section titles
  - body copy and descriptions
  - navigation labels and destinations
  - table content and frequently read values

- Emphasis should be created using font weight (`font-medium`, `font-semibold`) while remaining `text-fg`.
- Do not use muted text as a substitute for proper hierarchy or spacing.

- Muted text (`text-muted-fg`) must be used sparingly and intentionally.
  Allowed uses include:
  - helper text or hints
  - metadata (timestamps, counts, secondary labels)
  - short explanatory notes that are clearly secondary

- Muted text must remain fully readable and should never appear disabled unless the content is actually disabled.
- Do not apply muted text to entire paragraphs that carry meaning.
- Do not stack muted text on muted backgrounds.
- Avoid using muted text to reduce visual weight of primary content or layout elements.
...
- If more than a small minority of visible text on a page is muted, reassess the hierarchy. Muted text should never dominate a view.
...
- It is ok to pass spacing and layout `className` values to `Card` (for example padding, margin, gap, flex, grid, width, height, alignment, positioning).
- Do not pass any color or palette defining classes to `Card` (no `bg-*`, `text-*`, `border-*`, `ring-*`, `shadow-*`, `divide-*` that affect color). Colors must come from the shared components and semantic tokens only.
...
- It is ok to pass spacing and layout `className` values to `SubCard` (for example padding, margin, gap, flex, grid, width, height, alignment, positioning).
- Do not pass any color or palette defining classes to `SubCard` (no `bg-*`, `text-*`, `border-*`, `ring-*`, `shadow-*`, `divide-*` that affect color). Colors must come from the `SubCard` semantic role.
