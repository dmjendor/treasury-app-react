// app/@modal/(.)public/vaults/[vaultId]/transfer/valuables/[valuableId]/page.js
...
import { getValuableForVaultById } from "@/app/_lib/data/valuables.data";
...
/**
 * Render the transfer valuable modal.
...
export default async function Page({ params }) {
  ...
  if (!session?.user?.userId) {
    return (
      <Modal title="Transfer valuable">
        <div className="p-6 text-fg text-sm text-muted-fg">
  ...
  const [vault, valuable, permRes, transferRes] = await Promise.all([
    getVaultById(vaultId),
    getValuableForVaultById(vaultId, valuableId),
